...
## 2026-08-13 — Repo hygiene pass, requirements.txt audit, Turn Trace diagram, fine-tuning re-verified closed
- **SQL Server migration**: scoped what moving `ScheduleRepository` from SQLite to SQL Server would take (driver swap, `LIMIT`→`TOP`/`OFFSET FETCH`, `db_Tech.sql` is already the canonical T-SQL schema so no new seed script needed). Confirmed the local machine already has everything but a target DB: `pyodbc`/`SQLAlchemy` installed, ODBC Driver 17/18 present, a running `localhost\SQLEXPRESS` instance reachable via Windows Auth. **Paused before touching code** — user wants to confirm with the course instructor whether SQL Server is actually required before committing to the migration. No code changed; a throwaway branch was created and discarded.
- **Repo hygiene**: found and removed `TO_SUBMIT/` (62MB, a stale hand-copied duplicate of `app/`/`docs/`/`tests/`/`streamlit_app/` plus the full rebuildable `data/chroma` index — created by mistake in a different chat session, per the user). Also found `genai-recruiter-bot - Copy/` one directory up (214MB, a second full clone with its own `.git`/`.venv`) — investigated and confirmed it carries zero unique history (identical HEAD and `origin` to this repo) before flagging it for deletion; awaiting the user's go-ahead. Built a clean replacement submission folder (`../genai-recruiter-bot-submission`, 76 files, 1.5MB) via `git archive HEAD`, which — unlike a hand copy — can't drift out of sync or accidentally include `.venv`/`data/chroma`/`tech.db`. Also found the previously-existing `v1.2-submission.zip` (23 Jul) is 2 commits stale (missing the Streamlit UI polish + this session's cleanup commit) — flagged, not regenerated yet.
- **`requirements.txt` audit**: 4 pinned packages have zero imports anywhere in the codebase — `langgraph` (project deliberately never adopted the `langgraph` `StateGraph` API), `langchain`, `langchain-community`, `langchain-openai` (the project calls the OpenAI SDK directly via `app/llm_client.py`, not through langchain's wrapper). Meanwhile `langchain_core.tools` and `langchain_text_splitters` genuinely are imported (`sched_advisor/tools.py`, `embedding/build_index.py`) but aren't pinned directly — currently only installed as transitive deps of the packages slated for removal. Fix identified, not yet applied (awaiting go-ahead): drop the 4 dead packages, add `langchain-core`/`langchain-text-splitters` explicitly. The already-documented `jupyter`/`matplotlib`/`seaborn`/`scikit-learn` eval-only-package split (README's own "Cost/footprint note not yet addressed") is still open too.
- **`docs/turn_trace_diagram.html`**: new self-contained interactive artifact (no build step, opens directly in a browser) visualizing the turn-flow contract in `docs/one_turn_flowchart.json` — a static system map (who talks to whom, R-2/R-3/R-4) plus a step-through/auto-play trace over 4 real scenarios (info Q&A, scheduling, decline→end, and a multi-consult example demonstrating guard R-1), with a live consult-count meter and action-label readout. For presenting the project, not for the pipeline.
- **Fine-tuning platform closure, now definitively confirmed** (upgrades the 2026-07-19 finding from "blocked, maybe temporary" to "confirmed permanently closed, 3 independent ways"): re-ran `python -m app.modules.fine_tuning.launch_job launch` for real — training/validation files uploaded successfully, job creation still 403's, now with OpenAI's own explicit wording ("OpenAI is winding down the fine-tuning platform and your organization is no longer able to create new fine-tuning training jobs"). User separately checked the OpenAI dashboard UI (not just the API) and got the same closure message ("Fine-tuning training is no longer available for new organizations"). Discussed alternatives (Azure OpenAI fine-tuning, open-weight model + local/hosted fine-tuning) — user chose to leave the current prompted-only outcome as final rather than pursue either, so no architecture change follows from this.

## 2026-08-20 — GRB-041 closed: real `langgraph` `StateGraph` engine added alongside the legacy graph, verified behavior-identical
- **Instructor clarifications, resolving two standing open questions**: fine-tuning is confirmed **not required** by the course brief (Epic E3's prompted-only outcome is therefore full compliance, not a documented workaround); the SQLite port (spec A-1) is **approved**, closing the 2026-08-13 "confirm with instructor before migrating to SQL Server" open item — no SQL Server work needed.
- **Extracted and read the actual official brief** (`docs/GenAI_Project.pdf` — like the job-description PDF, this is a zip-of-page-text/images bundle, not a real PDF; same format quirk documented under GRB-012). Finding: the brief never mentions `langgraph` anywhere — it says "Agent Architecture: With LangChain & OpenAI API Endpoint and Models" and "LangChain – Agents, Memories, Tool". The `PROJECT_SPECIFICATION.md` §3.3 "LangGraph mandated" language is a prior Claude Code session's own architectural interpretation of the provided workflow diagram, not a literal brief requirement — worth knowing before treating spec §3.3 as gospel on this point. (Raised as a caveat; user's instructor separately said to proceed with the implementation anyway.)
- **Implementation, done as a strictly additive, side-by-side second engine** (not a replacement) — `app/graph.py` (the legacy plain-Python control flow) is untouched, byte-for-byte:
  - `app/graph_langgraph.py`: a real `langgraph` `StateGraph` (`route` node + `exit_advisor`/`sched_advisor`/`info_advisor` nodes, conditional edges implementing guard R-1's consult loop and the `sched_deferred_this_turn`/`experience_shared_this_turn` same-turn bookkeeping). Deliberately reuses `app.graph`'s private helpers (`_looks_meaningless`, `_in_schedule_phase`, `_pending_offered_slots`, `_synthesize`, ...) verbatim rather than reimplementing them — only the orchestration mechanism differs between the two engines, not the business logic, which is what makes exact behavioral parity achievable rather than just hoped-for.
  - `app/turn_engine.py`: a one-function dispatcher reading the new `settings.turn_engine` (`"legacy"` default | `"langgraph"`) and importing whichever engine's `run_turn` lazily. `app/main.py`, `streamlit_app/streamlit_main.py`, `tests/eval_replay.py`, `tests/verify_canonical_flows.py` now import `run_turn` from here instead of `app.graph` directly — a one-line-per-file mechanical change, so switching engines everywhere is one env var (`TURN_ENGINE=langgraph`), not a code change or a git branch switch. `tests/test_graph.py` (legacy-specific) is untouched.
  - `tests/test_graph_langgraph.py`: a near-verbatim port of all 21 `test_graph.py` cases retargeted at the new engine via a single `from app import graph_langgraph as graph` alias — every test body is character-identical, which is itself the point (same scenario, same assertion, both engines must agree). Plus 2 new tests confirming the compiled graph has the expected real nodes and that `get_graph().draw_mermaid()` produces real Mermaid source from the actual graph (useful later for docs/presentation — generated, not hand-drawn).
  - `requirements.txt`: `langgraph` pin bumped `0.5.4` → `1.2.11` (the version actually installed/tested here — the old pin predates this project ever having a real `langgraph` import, per the 2026-08-13 dead-deps audit, and is old enough it wasn't confirmed compatible with the `langchain-core` 1.5.2 already present for `sched_advisor/tools.py`'s `@tool` usage).
- **Verification, real API calls, not just mocks** (git branch `feat/langgraph-orchestration`, `main` untouched throughout):
  1. `pytest`: full suite green (151 tests, up from 121 — 30 new), `ruff check .` / `ruff format --check .` clean.
  2. `python -m tests.verify_canonical_flows` with `TURN_ENGINE=langgraph`: all 4 canonical flows pass against the real API (Q&A, scheduling incl. "next Friday" relative-date resolution, refusal→end, opt-out→end).
  3. **The real merge gate**: `data/tech.db` rebuilt fresh, full `tests.eval_replay` (sequential, all 44 graded turns, real API) run on legacy — **65.9% raw / 82.9% adjusted**, matching the documented CORE-REV baseline exactly. DB rebuilt fresh again, same eval run on `TURN_ENGINE=langgraph` — **65.9% raw / 82.9% adjusted**, and `diff`ing the two runs' full miss lists shows **zero differences**: the same 15 misclassified turns, same predicted/gold labels, same order. Two independent full real-dataset runs, byte-identical outcome — the strongest evidence available that this is a genuine behavior-preserving refactor, not just unit-test parity.
- **Still on `feat/langgraph-orchestration`, not yet merged to `main`** — pending the user's final call now that verification is complete. `settings.turn_engine` defaults to `"legacy"`, so nothing about default behavior changes even after merging; the value of merging now is just making the second engine available via the env var everywhere (CLI/Streamlit/eval) without a branch switch.
