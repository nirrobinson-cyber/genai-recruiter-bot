...
| 063 | Deploy | 🟡 | App verified locally (real `streamlit run` boot + full AppTest-driven interaction incl. a real API call). **2026-07-21: closed the data-bootstrap gap** — `streamlit_main.py` now auto-builds `data/tech.db`/`data/chroma/` on first run if missing (Community Cloud clones a fresh, ephemeral checkout with no shell access to run the setup scripts manually), verified via AppTest with the real data files moved aside. Full runbook in README's "Live deployment (GRB-063)" section. Actual Streamlit Community Cloud connection still requires the user's own account/GitHub push — not something that can be done from here |
