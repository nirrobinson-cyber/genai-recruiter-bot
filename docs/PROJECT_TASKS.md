...
| CORE-REV | **Core-flow revision** — booking-completion path + proactive escalation + slot rendering, target ≥75% eval accuracy | 🟡 | Ordinal/partial slot confirmation ("the second one", "Tuesday at 10 AM works") verified working live (3 independent test paths); meaningless-input guard ("f") shipped and tested. **Eval harness rebuilt to replay sequentially** (`tests/eval_replay.py --mode sequential`, default; `--mode isolated` kept for comparison), **plus divergence-artifact tagging** (raw vs. adjusted accuracy, every miss tagged GENUINE/DIVERGENCE). **Current (2026-07-20, cont. 2): raw 56.8% (25/44), adjusted 73.5% (25/34)** excluding divergence artifacts — up from 52.3%/67.6% earlier the same day. A numeral-years-of-experience date-parsing bug fixed (real win, kept). Same day: 3 narrow Sched/Exit Advisor bugs fixed (verified via real-API tests) but didn't move the aggregate score (a separate labeling gap + sequential cascading offset them) — then re-examining the "genuinely inconsistent ground truth" escalation-timing claim across all 15 conversations (not just the 2 originally compared) found it was real ambiguity but NOT unresolvable: 10/15 conversations schedule immediately on the first qualifying reply (majority), only 5/15 defer, and a real content-based signal (general background statement vs. naming a specific new technology) separates them 8/8 clean under the dataset's most common opener. Fixed the routing prompt (`app/modules/main_agent/prompts.py`) accordingly — this IS a genuine gain, unlike the same-day Sched/Exit fixes. One known remaining gap (a compound-sentence case, conversation 3) kept as a documented `xfail`; one foreseen new loss (conversation 4, the one opener-group exception) accepted as an explicit trade-off rather than overfitting the prompt to one conversation. Still below the ≥75% adjusted target (73.5%, close) and the spec's 85% — the earlier blanket same-turn guard attempt (reverted, schedule recall 78.9%→31.6%) remains the cautionary tale for why this fix stayed narrow and content-based rather than another blanket rule. |
