...
| 033 | Baseline comparison (fine-tuned vs prompted) | ✅ | prompted-only comparison run for real (end: P=0.50 R=0.33 F1=0.40, n=10 val); fine-tuned row can't be produced on this account (OpenAI platform deprecation, EXT-1). Per this epic's own AC ("fine-tuned wins on end recall, or the fallback decision is documented — either outcome closes the epic"), the documented fallback satisfies it — Exit Advisor stays on the prompted path |
| 040 | ConversationState | ✅ | |
| 041 | Turn graph | 🟡 | Legacy plain-Python control flow (`app/graph.py`, default) is behaviorally complete. A real `langgraph` `StateGraph` engine (`app/graph_langgraph.py`) now exists side-by-side, selectable via `settings.turn_engine`/`app/turn_engine.py` (2026-08-20) — verified behavior-identical to legacy via 2 full real-API eval runs (65.9%/82.9% both engines, zero diff in misses) after full DB reseeds between runs. On branch `feat/langgraph-orchestration`, not yet merged to `main`. Note: the actual course brief (`docs/GenAI_Project.pdf`) never names `langgraph` specifically — only "LangChain — Agents, Memories, Tool"; the §3.3 "LangGraph mandated" wording is this project's own prior interpretation, not a literal brief requirement (see DEVLOG 2026-08-20). |
...
| 063 | Deploy | ✅ | **Live: https://genai-recruiter-bot-l3q65zqqp2u4d26wph2xly.streamlit.app/** — deployed to Streamlit Community Cloud (public and searchable), user-verified end-to-end (registration, real chat turn with bot reply, dev trace panel, Reset). Data-bootstrap gap closed 2026-07-21 (`streamlit_main.py` auto-builds `data/tech.db`/`data/chroma/` on first run if missing, since Community Cloud clones a fresh ephemeral checkout with no shell access) — verified via AppTest before the real deploy. Full runbook in README's "Live deployment" section |
...
| CORE-REV | **Core-flow revision** — booking-completion path + proactive escalation + slot rendering, target ≥75% eval accuracy | ✅ | **Target met (2026-07-21): raw 65.9% (29/44), adjusted 82.9% (29/35)** excluding divergence artifacts — first time this project has exceeded the ≥75% adjusted target. Meaningless-input guard ("f") shipped and tested. Eval harness replays sequentially by default with GENUINE/DIVERGENCE miss tagging (`tests/eval_replay.py`). Path to target: numeral-years-of-experience date-parsing fix → 3 narrow Sched/Exit Advisor fixes (2026-07-20, didn't move the aggregate alone) → routing-prompt fix re-deriving the escalation-timing pattern from all 15 conversations instead of 2 (2026-07-20 cont. 2, +4.5/+5.9pp) → same-turn "double-consult inconsistency" fix in `app/graph.py` (2026-07-21, +9.1/+9.4pp, crossed the target). A separately-investigated "slot confirmation" issue (conversations 6/7) turned out not to be a code bug at all — our system's real-time nearest-available-slot offering structurally can't always match a synthetic dataset's fixed script; confirmed via full replay + a DB rebuild, correctly tagged DIVERGENCE by the harness, no fix applied. Still below the spec's 85% target; remaining named gaps (compound-sentence routing edge case, one opener-group exception, the genuinely-inconsistent ground-truth pattern) are documented `xfail`s or accepted trade-offs, not chased further per the 78.9%→31.6% regression lesson from the one blanket-rule attempt that was tried and reverted. **CLOSED for v1.1 (2026-07-21), by explicit user decision:** BUG-1 (ordinal/partial slot confirmation, e.g. "the second one") and BUG-2 (the `continue`-vs-`schedule` action-labeling gap from the 2026-07-20 Sched-confirmation fix). Both accepted as known limitations, not chased further. **BUG-1 status update (2026-07-21, later same day):** genuinely reproduced via a user-supplied live transcript ("ok choose 1" after 3 unnumbered slots were offered) — root cause confirmed via a clean isolated replay: `CONFIRMATION_PROMPT_ADDENDUM` (`app/modules/sched_advisor/advisor.py`) has no instruction or example for numeric/positional references, so the model understands the intent (its own returned reason was literally "Candidate accepted a proposed time") but returns `decision="sched"` instead of `"confirmed"`, which falls through to the plain date-lookup path and silently offers a *different* batch of slots instead of booking the one the candidate picked. A concrete fix was scoped (number the offered slots in the message text; teach the confirmation prompt to match a bare number to list position, additive alongside existing free-text matching) but deliberately **not implemented** — user's explicit call, still closed for this submission. Also found in passing, not part of this bug: `date_resolver`'s month-name pattern silently drops a trailing day when phrased "september 2024 14" (defaults to the 1st) — a separate, smaller, unfixed gap. Sched Advisor's bare "sep 2024" (month+year, no day) classification also showed real turn-to-turn inconsistency across two separate real-API calls with identical input — likely LLM sampling variance at a genuinely ambiguous boundary case, not chased. BUG-2 remains unreproduced/unchanged. **If either resurfaces again beyond what's documented here, open it as a new, separate issue.** |
