"""Tests for the LangGraph `StateGraph` turn engine (GRB-041 dual-implementation).

This file is deliberately a near-verbatim port of `tests/test_graph.py`,
retargeted at `app.graph_langgraph` via the `graph = graph_langgraph` alias
below — every test body is unchanged. The point is exact behavioral parity:
if a scenario is asserted here and in test_graph.py, both turn engines must
produce the identical result for it. Advisor/routing decisions are mocked —
no real LLM/DB calls here."""
...
from app import graph_langgraph as graph
...
def test_run_turn_second_consecutive_continue_still_restates_offered_slots(
    ...
    assert seen_phase_hints[0] == "schedule"
    assert result["action"] == "continue"
    assert "2024-04-16" in result["message"]
...
# --- LangGraph-specific: confirms this is a real StateGraph, not a relabeled loop ---


def test_compiled_graph_has_the_expected_nodes() -> None:
    node_names = set(graph._COMPILED_GRAPH.get_graph().nodes)
    assert {"route", "exit_advisor", "sched_advisor", "info_advisor"} <= node_names


def test_mermaid_diagram_is_generated_from_the_real_graph() -> None:
    diagram = graph.mermaid_diagram()
    assert "route" in diagram
    assert "sched_advisor" in diagram
